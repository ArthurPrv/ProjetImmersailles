...
include("../html/connexion_bdd.php");
    ...
    <label>Année: </label>
        ...
        <input id='Annee' type="range" list="numbers" onchange='this.form.submit()' ondrag="updateTextInput(this.value);" >
        ...
        <datalist id="numbers" >
            ...
            while ($ligne = $results->fetch()) {
                if($Annee==$ligne['Annee']){echo "<option  select = selected value='" . $ligne['Annee'] . "'   ></option>";}
                else{  echo "<option  value='" . $ligne['Annee'] . "'   ></option>";}
            }
            $results->closeCursor();
            ?>
    ...
    <select name='Niveau' id='Niveau' onchange='this.form.submit()'>"
        ...
        while ($ligne2 = $results->fetch()) {
            ...
            else {echo "<option value=\"" . $ligne2['Niveaux'] . "\">" . $ligne2['Niveaux'] . "</option>";}
        }
        $results->closeCursor();
...
<script >
    ...
    map.on("contextmenu", function (event) {
        ...
        marker.on('dblclick', function(){marker.remove(this);})
        marker.addTo(map);
